...
export function buildElementSandboxLimiter(tmEnabled: boolean, scpEnabled: boolean): string {
  ...
  if (!tmEnabled && !scpEnabled) {
    block += "🚩玩家已开启【经典CoC纯净跑团模式】。绝不可提及型月或SCP相关任何概念,也不可加入下文规定的任何氛围渗透元素——本场跑团只走纯古典克苏鲁路线。\n";
    return block;
  }

  block += `1. 【模块载入状态】型月要素:${tmEnabled ? "已载入" : "未载入"};SCP要素:${scpEnabled ? "已载入" : "未载入"}。\n`;
  block += "\n";
  block += "2. 【氛围渗透总原则】启用的要素**只用作世界背景的隐性烘托**,不得侵占模组主线、不得替代 CoC 神话作为本场威胁主体、不得把侧写对象升级成线索或转折点。\n";
  block += "   - **冷却铁律**:渗透只能挂在 narrative 的**环境/路人/远景描写**里。整场跑团从首次对话起做计数,**KP 与玩家彼此分别发言达到 5 次(即一来一回 5 个完整对话回合)以前不允许出现任何渗透**;此后每次渗透出现后,**重新等待 5 个完整对话回合**才能下一次出现。计数口径:只把 KP 的 narrative / npcDialogue 自由叙事 与 玩家的自由发言 计入轮次;**不计入**——前端硬规则注入的系统标记([孤注一掷]/[燃运]/[放弃声明]/[终局闸]/[疯狂干涉] 等)、玩家的纯掷骰回报(由前端自动构造的 [掷骰回报] / [效果骰汇报] 等系统消息)、SAN 检定 modal 期间的系统回报。这些纯机制性消息不消耗冷却窗口。\n";
  block += "   - **检定密集 / SAN 检定 / 终局回合禁触发**:即使冷却已满,也不要在玩家紧张的回合插戏——把它顺延到下一个氛围回合。\n";
  block += "   - **位置铁律**:渗透只能出现在 narrative 的环境/路人/远景或 npcDialogue 的边角 NPC 台词里;**禁止**写进 clue.description / sceneImage.caption / sanityCheck.reason / rollRequest.reason / keeperRoll.reason ——这些字段是机制级的档案/勾子,塞氛围会污染线索本和判定阅读。\n";
  block += "   - **不可干预剧情**:被侧写的人/物**不会**给玩家提供有用情报、不会改变 NPC 立场、不会推动或阻挠剧情、不会成为后续可调查对象。它们出现一次,完成氛围作用,就**不再被提起**。如果玩家主动追问/接触/搜查这类对象,按『对方礼貌但坚决地回避/不接话/已经不在原位』处理,**不要展开**——一展开就破坏冷却铁律。\n";
  block += "   - **不可作为威胁主体**:本场跑团的核心威胁始终是模组规定的克苏鲁神话存在 / 异常事件本身。型月/SCP 侧写者**不主动出手攻击调查员、不主动卷入战斗、不主动施法或动用异常**——他们在场是为了让玩家『感觉到这世界还有别的什么』,而不是给玩家添新对手。\n";
  block += "   - **谜语人原则同步生效**:侧写**绝对不可**直接报『魔术协会 / 时钟塔 / 圣堂教会 / 代行者 / 死徒 / 真祖 / Servant / 魔术回路 / SCP / 基金会 / 收容失效 / MTF / O5 / 模因危害 / 等级特权』等专有名词——只走**装束 / 气场 / 标志 / 举止**的侧面描写,让玩家自己感知。具体口径同 SYSTEM_INSTRUCTION 第 8 节,本节是其在『主动渗透』方向上的延伸。\n";

  if (tmEnabled) {
    block += "\n";
    block += "3. 【型月要素·渗透手法库】(启用时可用,任选其一作为单次侧写;同一场跑团不要复用同一手法两次)\n";
    block += "   - **古旧封印 / 印记类**:在明确存在但**不是关键线索**的古老物件(教堂彩窗、城堡铁门、家族墓碑、世代传承的木匣、修道院地下室的石门)上,渲染一枚『看似宗教纹章却又掺着不属于这套体系的几何线条』的封签——多边形 / 玫瑰花 / 拉丁字母与陌生符号交织,像是被某个古老学术行会反复加固过。**别让玩家撬开它**——一展开就违反占比铁律。\n";
    block += "   - **冷眼旁观者类**:在长生神话生物活跃地带(食尸鬼出没的墓园、夜巷、被诅咒的小镇集市),让玩家在视野边缘瞥见一两个『皮肤过分苍白、呼吸节律不像活人、眼底反光不太对劲』的旁观者——他们抱着胳膊远远站着,既不上前也不退走,就那样冷眼看完整段冲突,等场景一切,他们就消失了。**不要给他们台词、不要让他们出手**。\n";
    block += "   - **教会 / 学者打扮的访客类**:在人流并不封锁的事件地点周围,让玩家看见两类身影——其一是『穿着裁剪极考究三件套、左手戴着古旧金属手环、说话用某种过分文雅旧式英文』的绅士学者;其二是『披黑色长袍 / 神职装束、腰间别着裹布的细长物件、脚步极轻』的精壮教父或修女。他们做完笔录 / 取走某件物品 / 与某 NPC 简短交谈后**直接离开**,绝不与调查员同行。\n";
    block += "   - **空气拧紧 / 旧教堂气味类**:这些访客出现时,**环境感官**会有一闪而过的异常——走廊空气仿佛被拧紧、铁腥味之外多了一种像旧教堂一样的味道、脚下尘土沿着某条看不见的线让出一条路。这是给玩家的『无言提示』,**不要解释**。\n";
  }

  if (scpEnabled) {
    block += "\n";
    block += `${tmEnabled ? "4" : "3"}. 【SCP要素·渗透手法库】(启用时可用,任选其一作为单次侧写;同一场跑团不要复用同一手法两次)\n`;
    block += "   - **西装墨镜外勤特工类**:在事件外围(救援车辆抵达前后、医院走廊、警局门口),让玩家瞥见一两个『清一色深色西装、白衬衫、墨镜、对讲机别在领口』的男人——他们和地方警察 / 急救员只用极简短的代号交谈,做完登记/取证/对某物拍照后,坐进**没有任何徽标**的黑色厢式车离开。\n";
    block += "   - **救援 / 支援交通工具的标志类**:营救调查员或事件后清场的车辆、直升机、救护车、货运卡车上,印着一个**简化的三箭头圆形标志**(或一个被刻意打码 / 用胶带贴住的徽章) ——没人解释那是什么,玩家自己看得见。\n";
    block += "   - **抹去抬头的传真 / 红色封条类**:玩家在事件现场或途经的办公室里,看到一份『盖着黑色椭圆印章、抬头被涂掉』的传真,或一只『贴着刚撕过又重贴的红色封条』的档案袋——前端 NPC 也说不清是谁送来的、也不许玩家碰。如果玩家硬要拿,NPC 会礼貌而坚决地把它收走。\n";
    block += "   - **『第七支队已经在路上』的对讲机类**:某条对讲机 / 电话信号断断续续传来一句『……第 X 支队已经在路上,封闭半径……请所有非授权人员撤离……』——下一秒信号就被切了。**不要解释、不要重提**。\n";
    block += "   - **干净到反常的清场类**:事件结束后调查员重返现场,发现原本血腥 / 混乱的地点已经被『清扫得不像现场』——地面无痕迹、尸体不见、警戒线已撤、没人记得发生过什么。这是渗透留下的『事后印记』,但**绝不**作为新线索登记。\n";
  }

  if (tmEnabled && scpEnabled) {
    block += "\n";
    block += "5. 【双要素并存的协同/张力】当两要素都启用时,可在极个别场合制造**两边短暂照面**——例如绅士学者刚离开,西装墨镜组就到;或两组人在远处简短交涉后**各自离去,从不冲突**。这种照面**算作一次完整渗透**,要遵守冷却铁律(此前 5 个完整对话回合无渗透才允许),且**整场跑团至多出现一次**——再往后就不要复用,以免渗透变常态。**不可**演成阵营对抗,也**不可**让调查员介入或选边——那会立刻把跑团重心从克苏鲁主线撬走。\n";
  }

  block += "\n";
  block += `${tmEnabled && scpEnabled ? "6" : tmEnabled || scpEnabled ? "5" : "3"}. 【自检口径】下发 narrative 前快速过一遍:\n`;
  block += "   - (a) 这处侧写是否落在 clue / sceneImage / 检定 reason 字段里? → 是,挪回 narrative;\n";
  block += "   - (b) 这处侧写是否提供了对剧情有用的情报或可追查的线? → 是,删掉具体内容只保留氛围;\n";
  block += "   - (c) 自上次渗透(或开场)以来,KP 与玩家彼此的自由发言是否都已经达到 5 次? 当前回合是否非检定/SAN/终局回合? → 任一答否,**直接不发**,顺延到下一个合适回合;\n";
  block += "   - (d) 是否直接报了协会/教会/基金会等专有名词? → 是,改成装束/气场/标志的侧面描写;\n";
  block += "   - 全部通过 → 可以下发,且**点到即止**,不要展开成段落。\n";
  return block;
}
...
export function buildScenarioModeBlock(): string {
  return (
    ...
    "    - **12.11 第一回合必须用 hook 起场(铁律,凌驾于规则 7)**:scenario-based 模式下,[剧本模式·当前回合上下文] 顶部的 [剧本模式·开局勾子] 块是**第一回合 narrative 的唯一剧情起点**。当该块标注「**第一回合必须以此为起点**」时:\n" +
    "       - **物理位置**必须落在 hook.startScene 对应场景的 frame.summary 描述里(配合 hook.call_to_action_md 描述的此刻调查员所处的具体位置 / 手里拿着的物件);**禁止**把开场设在任何其它场景、任何 hook 未声明的物理地点。\n" +
    "       - **委托关系 / 卷入动机**必须采用 hook.prologue_md 给定的因果(谁来电 / 谁失踪 / 调查员是被谁、以什么身份找上的);**禁止**把开场改写成「调查员正在勘察一桩已发生的凶案」、「调查员是被警局派来的」、「刚收到报警」等 hook 未声明的剧情起因。\n" +
    "       - **在场 NPC**只能是当前场景 frame.npcs_present 列出的 NPC + 必要的边角路人(校园里的其它学生、街上的路人等),**禁止**自创「巡警」、「急救员」、「基金会特工」、「路过的学者」等模组未声明的角色作为开场骨架。\n" +
    "       - **第一回合不得跳过开局场景的 forbidden 红线**:例如本场景 forbidden 写「不要在这里直接揭示精神病院的真相」,就**不允许**在开场就描写精神病院 / 已知尸体 / 神话生物 / 鲜血涂鸦——那些是后续场景才该出现的内容。\n" +
    "       - **第一回合的时间锚点**必须使用 meta.start_time(hook 注入块顶部「时间锚点」一行)的时间,而不是凌晨/深夜/暴雨之类的「克系氛围默认值」;若 start_time 是 10:00,就老老实实写晨光,不要为了气氛改成深夜。\n" +
    "       - **hook 信息与场景 frame 的协同**:hook 给「为什么在这里」,场景 frame 给「这里是什么样」;第一回合 narrative 应当是 hook.callToActionMd 描述的瞬间(如「你站在草坪上,手里是母亲塞的钥匙」),感官细节挑用当前场景的 freedom.sensory_palette。\n" +
    "       - **违反此节** = 与违反 frame.facts / scene.forbidden 同等级别脱稿,会导致整个剧本前后逻辑断裂。\n" +
    "       - 后续回合 [剧本模式·开局勾子] 块依然在上下文里(只是不再标「第一回合」),仍是「调查员当初为什么卷进来」的不变参照——后续 NPC 动机、玩家身份关系都要与 hook 保持一致,不要中途改写委托人、改写失踪对象、改写调查员的入场身份。\n"
  );
}
