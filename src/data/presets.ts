 ...
 *
 * 调查员预设角色模板（1920s × 10 + modern × 10）。
 *
 * 字段对齐 .docs/character-dictionary.yaml：
 * - skills 键名严格使用 SKILL_REGISTRY_ALL 的 nameZh，多分支技能用 "父(分支)"。
 * - 不再把 era 前缀塞进 occupation 字段；era 由 background 携带。
 * - 创建期默认：mythicEncounters 四类空、inventory 8 个空槽、sanityState 初值、
 *   cashBalance 由 startingCashOf 派生写入；combatDerived 由 handleSelectPreset
 *   通过 refreshCombatDerived 现算补写。
 *
 * 注：起始现金口径见 cocRules.startingCashOf —— 1920s = CR×1，modern = CR×20。
 */
import { CharacterSheet, MythicEncounters, InventoryEntry } from "../types";

const EMPTY_INVENTORY: InventoryEntry[] = Array.from({ length: 8 }, () => ({ kind: "item" as const, text: "" }));
...
export const TEMPLATE_PRESETS: (CharacterSheet & { backgroundText?: string })[] = [
  ...
  {
    name: "尤利娅·阿尼西莫娃 (Yulia)",
    occupation: "军官",
    identity: "圣堂教会东欧分支行刑代行者",
    nationality: "俄罗斯",
    residence: "布达佩斯, 多瑙河左岸",
    motherTongue: "俄语",
    creditRating: 50,
    gender: "女",
    age: 28,
