...
if (result.ok === false) {
  ...
  throw new Error(`[scenario:one-nest-of-trouble] 校验失败,这不应发生(prebuild 已校验过):\n${lines}`);
}
if (result.warnings.length > 0) {
  // 走 console.warn 不阻塞,Phase 1 prebuild 已经看过这些 warning
  console.warn(
    `[scenario:one-nest-of-trouble] 校验通过但有 ${result.warnings.length} 条 warning:`,
    result.warnings,
  );
}
